...
struct worldCoords
{
    ...
    void update(double newY, double newX)
    {
        y = newY;
        x = newX;
    }
...
void loop()
{
    ...
    int16_t accelData[3] = {0};  // X, Y, Z acceleration
    int16_t gyroData[3] = {0};   // X, Y, Z gyroscope

    // Correct function calls using the required array format
    ...
    locus.update(37.27, -121.83); // Pass appropriate values for latitude, longitude, and altitude
}

void Emergency()
{
    //send all information into Azure IOT
}
